<div class="card content h-full flex flex-col" x-data="{
     ...
     <div class="table-header flex justify-between items-center">
          ...
          <div class="flex items-center gap-3">
               ...
               <form id="asset-import-form" action="/assets/import" method="POST" enctype="multipart/form-data">
                    ...
                    <div id="asset-import-button" title="Import Assets">
                         <i class="fa-solid fa-file-import cursor-pointer"></i>
                    </div>
               </form>

               <i class="fa-solid fa-file-export cursor-pointer" @click="showModal = true; modalTemplate = 'export-filter'" title="Export Assets"></i>

               <i class="fa-solid fa-clipboard-list cursor-pointer" @click="showModal = true; modalTemplate = 'audit-log-filter'" title="Export Audit Log"></i>
               ...
               <div x-data="{ filterOpen: false }" class="relative">
                    ...
                    >
                         <div class="flex items-center justify-between p-4 border-b border-gray-200">
                              ...
                              <button @click="filterOpen = false" class="text-teal-500 hover:text-teal-600 transition">
                         ...
                         <div class="flex items-center justify-between p-4 border-t border-gray-200">
                              <button wire:click="resetFilters" class="px-4 py-2 text-xs font-bold text-gray-600 hover:text-gray-800">Reset</button>
                              <button @click="filterOpen = false" class="px-5 py-2 bg-teal-500 text-white rounded-lg text-xs font-bold hover:bg-teal-600">Apply Filter</button>
                         </div>
                    </div>
               </div>
          </div>
                         ...
                         <tr>
                              ...
                              <td>
                                   <p class="flex items-center gap-2">
                                        <img src="{{ asset('img/' . $categoryCodeImage[$asset->category]->icon . '.png') }}" style="width: 25px" alt="">
                                        <span class="font-bold">{{ $categoryCodeImage[$asset->category]->name }}</span>
                                   </p>
                              </td>
                              ...
                              <td x-data="{ open: false }" class="relative">
                                   ...
                                   <div x-show="open" @click.outside="open = false" x-transition class="absolute right-0 mt-2 w-36 bg-white border border-gray-200 rounded-lg shadow-md z-40">
     ...
     {{-- Import Loading Backdrop (moved outside header row) --}}
     <div
     ...
     >
          <div class="relative bg-white p-10 rounded-lg shadow-lg w-full max-w-lg">
               <div class="absolute right-7 top-7 text-gray-400 cursor-pointer hover:text-gray-800" @click="showModal = false;">
                    <i class="fa-solid fa-xmark"></i>
               </div>

               {{-- Select Category --}}
               <div class="w-full" x-show="modalTemplate === 'create'">
                    <h3 class="text-center font-bold text-xl mb-6">Select Category</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8" x-data="{ openCat: null }">
                         @foreach($categories as $category)
                         <div>
                              <button
                              ...
                              >
                                   ...
                                   <i
                                        ...
                                        :style="openCat === {{ $category->id }} ? 'transform: rotate(180deg)' : 'transform: rotate(0deg)'"
                                   ></i>
                              </button>
                              ...
                              <div
                                   ...
                                   class="ml-8 mt-2 space-y-1 text-sm text-gray-600"
                              >
                                   @foreach($category->subcategories as $sub)
                                   <a
                                        href="{{ url('/assetmanagement/create?category_type=' . $sub->category_type . '&category=' . $category->code . '&sub_category=' . $sub->name) }}"
                                        class="flex justify-between items-center cursor-pointer text-gray-500 font-semibold hover:text-gray-800 hover:translate-x-1 transition-transform"
                                   >
                                        <span>{{ $sub->name }}</span>
               ...
               {{-- Delete Confirm --}}
               <div class="flex flex-col gap-5 w-full" x-show="modalTemplate === 'delete'">
               ...
               {{-- Export Assets --}}
               <div class="flex flex-col gap-5 w-full" x-show="modalTemplate === 'export-filter'">
               ...
               {{-- Export Audit Log --}}
               <div class="flex flex-col gap-5 w-full" x-show="modalTemplate === 'audit-log-filter'">
                    <h2 class="text-xl font-semibold -mb-2">Export Audit Log</h2>
                    <p class="text-sm text-gray-500">Filter audit records to export. Leave blank to export all.</p>
     ...
     {{-- Import Script (moved outside header, scoped to unique IDs) --}}
     <script>
          (function () {
